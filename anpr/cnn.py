...
def preprocess_char_for_cnn(char_roi):
    ...
    # Thêm padding viền đen
    padding = 5
    if char_roi.shape[0] > 0 and char_roi.shape[1] > 0:
        char_roi = cv2.copyMakeBorder(char_roi, padding, padding, padding, padding, cv2.BORDER_CONSTANT, value=0) 
    else:
        return None 
...
if screenCnt is not None:
    ...
    if plate_type == "square":
        ...
        if len(cnts1) > 0:
            ...
            # Dùng enumerate để biết vị trí ký tự (0, 1, 2...)
            for i, c in enumerate(cnts1_sorted):
                (x, y, w, h) = cv2.boundingRect(c)
                if h > 10 and w/h < 1.5: 
                    char_roi = row1_img[y:y+h, x:x+w]
                    char_cnn = preprocess_char_for_cnn(char_roi)
                    if char_cnn is None: continue 
                    
                    prediction = model.predict(char_cnn, verbose=0) 
                    predicted_label_index = np.argmax(prediction)
                    predicted_char = reverse_label_mapping.get(predicted_label_index, '?') 
                    
                    # === QUY TẮC: 2 ký tự đầu luôn là SỐ (Mã tỉnh) ===
                    if i < 2:
                        if predicted_char == 'Z': predicted_char = '2'
                        if predicted_char == 'T': predicted_char = '1'
        ...
        if len(cnts2) > 0:
            ...
            for c in cnts2_sorted:
                ...
                if h > 10 and w/h < 1.5:
                    ...
                    # === QUY TẮC: Dòng 2 biển vuông 100% là SỐ ===
                    if predicted_char == 'Z': predicted_char = '2'
                    if predicted_char == 'T': predicted_char = '1' 
        ...
        if len(cnts) > 0:
            ...
            for i, c in enumerate(cnts_sorted):
                ...
                # Biển dài ký tự thường cao hơn biển vuông
                if h > 20 and aspect_ratio < 1.5: 
                    ...
                    # === QUY TẮC: 2 ký tự đầu luôn là SỐ ===
                    if i < 2:
                        if predicted_char == 'Z': predicted_char = '2'
                        if predicted_char == 'T': predicted_char = '1'
